...
public class AccountDaoImpl implements AccountDao {
    ...
    @Override
    public GestoreAgenziaImmobiliareDTO checkAdminExists(GestoreAgenziaImmobiliare g, String psw) throws SQLException {
        String sql = "SELECT isfattoPrimoAccesso,g.password FROM GestoreAgenziaImmobiliare g WHERE g.username = ? AND g.nomeAgenzia = ? AND g.cf = ?  AND g.admin = ?";
        try(PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if(ciSonoRisultati(rs)){
                    ...
                    if(isPasswordGiusta(psw,rs)){
                        dto.setFattoPrimoAccesso(isFattoPrimoAccesso(rs));
                    }else{
    ...
    private boolean isFattoPrimoAccesso(ResultSet rs) throws SQLException {
        return rs.getBoolean("isfattoprimoAccesso");
    }
