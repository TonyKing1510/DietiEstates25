...
public class LoginAmministratoreController {
    ...
    public void initialize() {
        ...
        addListener(options);
        addListenerStopFocusDaCF();
        addListenerStopFocusDaUsername();
        addListenerStopFocusDaPassword();

    }

    private void addListenerStopFocusDaPassword() {
    ...
    private void addListenerStopFocusDaUsername() {
    ...
    private void addListenerStopFocusDaCF() {
        cfTextField.focusedProperty().addListener((observable, oldValue, newValue) -> {
           if(!newValue){
    ...
    private static boolean isAndatoTuttoBene(GestoreAgenziaImmobiliare g){
        return AuthServiceFacade.primoLoginGestore(g);
    }
    ...
    public void apriPaginaAccedi() {
        try{
            AccediAdmin.initializePageAccediAdmin(bottoneAccedi.getScene().getWindow());
        } catch (IOException e){
            AlertFactory.generateFailAlert("Errore nel caricamento", "Siamo spiacenti si è verificato un errore" +
                    "nel caricamento della pagina accedi,si prega di riprovare.Se il problema persiste contattare il supporto.");
            e.printStackTrace();
        }
    }
