...
import com.example.prova2.requester.AddAgenteRequester;
...
public class CreaAgenteImmobiliareController {
    ...
    private boolean inviaDatiEffettivo() {
        try {
            ...
            return AddAgenteRequester.addAgente(a);
        }catch (IOException e) {
    ...
    private boolean checkDuplicatoCF(String cf) {
        try {
            return CreaGestoreController.isCampoGiaRegistrato(cf, GestoreRequesterCF.getAllCfGestore(), erroreCodiceFiscale, "Le chiediamo gentilmente di inserire un codice fiscale non ancora registrato.");
        }catch (IOException e) {
            return false;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean checkDuplicatoUsername(String username) {
        try {
            return CreaGestoreController.isCampoGiaRegistrato(username, GestoreRequesterUsername.getAllUsernameGestore(), erroreUsername, "Le chiediamo gentilmente di inserire uno username non ancora registrato.");
        }catch (IOException e) {
            return false;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean checkDuplicatoEmail(String email) {
        try {
            return CreaGestoreController.isCampoGiaRegistrato(email, GestoreRequesterMail.getAllEmailGestore(), erroreEmail, "Le chiediamo gentilmente di inserire una mail non ancora registrata.");
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        } catch (InterruptedException e) {
