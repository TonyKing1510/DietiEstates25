...
import com.example.prova2.requester.GestoreRequesterCF;
import com.example.prova2.requester.GestoreRequesterMail;
import com.example.prova2.requester.GestoreRequesterUsername;
...
public class CreaGestoreController {
    ...
    public  void salvaDati() {
        if(checkCampi() && (checkDuplicati(textFieldEmail.getText(),textFieldUsername.getText(),textFieldCf.getText()))) {
                if (!bottoneCliccato) {
    ...
    private boolean exists(List<String> list, String object) {
    ...
    public boolean isCampoGiaRegistrato(String campo, List<String> campi, Label errore, String message){
        return isGiaEsistente(campo,campi,errore, message);
    }

    private boolean isGiaEsistente(String campo,List<String> lista,Label errore,String message)
    {
    ...
    private void scriviMessaggioDiErrore(Label errore, String message) {
    ...
    private boolean checkDuplicatoCF(String cf) {
        try {
            return isCampoGiaRegistrato(cf, GestoreRequesterCF.getAllCfGestore(), erroreCodiceFiscale, "Le chiediamo gentilmente di inserire un codice fiscale non ancora registrato.");
        }catch (IOException e) {
            return false;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean checkDuplicatoUsername(String username) {
        try {
            return isCampoGiaRegistrato(username, GestoreRequesterUsername.getAllUsernameGestore(), erroreUsername, "Le chiediamo gentilmente di inserire uno username non ancora registrato.");
        }catch (IOException e) {
            return false;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean checkDuplicatoEmail(String email) {
        try {
            return isCampoGiaRegistrato(email, GestoreRequesterMail.getAllEmailGestore(), erroreEmail, "Le chiediamo gentilmente di inserire una mail non ancora registrata.");
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        } catch (InterruptedException e) {
    ...
    public void tornaIndietro() throws IOException, InterruptedException {
        DashboardAmministratoreController.setAdmin(DashboardAmministratoreController.getAdmin());
