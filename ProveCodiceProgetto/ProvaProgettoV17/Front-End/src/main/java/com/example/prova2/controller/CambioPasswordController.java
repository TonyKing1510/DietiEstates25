...
public class CambioPasswordController {
    ...
    public void cambiaPassword(){
        if(checkCampi()){
            if(isCambioAvvenuto()) {
                Alert alert=AlertFactory.generateAlertSuccess("Torna alla home","Torna Dashboard","Operazione completata","La password è stata cambiata!","La password è stata cambiata con successo!");
                alert.showAndWait();
            }
            else{
                AlertFactory.generateFailAlert("Errore nel cambio della password","Errore nel cambio della password");
            }
        }
    }
