...
import com.example.prova2.view.LoginAdminAccedi;
...
public class LoginAmministratoreController {
    ...
    @FXML
    public void handleapriAccedi(){
        try {
            LoginAdminAccedi.initializePageLoginAdminAccedi(bottoneAccedi.getScene().getWindow());
        }catch (IOException e){
            e.printStackTrace();
            AlertFactory.generateFailAlert("Errore di navigazione",
                    "Si è verificato un problema durante il caricamento della dashboard.");
