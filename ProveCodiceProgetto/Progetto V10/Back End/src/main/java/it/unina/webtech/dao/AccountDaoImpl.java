...
public class AccountDaoImpl implements AccountDao {
    ...
    public boolean checkAdminExists(Admin admin) throws SQLException {
        String sql = "SELECT 1 FROM GestoreAgenziaImmobiliare g WHERE g.nome = ? AND g.cognome = ? AND g.nomeAgenzia = ? AND g.cf = ? AND g.email = ? AND g.admin = ?";
        try(PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, admin.getNome());
            ps.setString(2, admin.getCognome());
            ps.setString(3, admin.getAgenziaAppartenente().getNomeAgenzia());
            System.out.println(admin.getCf());
            ps.setString(4, admin.getCf());
