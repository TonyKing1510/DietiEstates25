...
import it.unina.webtech.model.AccountGestore;
...
public class GestoreDaoImpl implements GestoreDao{
    ...
    public boolean addGestore(GestoreAgenziaImmobiliare g) throws SQLException {
        String sql = "insert into GestoreAgenziaImmobiliare\n" +
                "(nome,cognome,cf,telefono,via,n°civico,username,password,email,admin,gestoricreati,nomeagenzia,isfattoprimoaccesso)\n" +
                "values\n" +
                "(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try( PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    public Map<String,Object> getGestoreByUsername(String username) throws SQLException {
        String sql = "SELECT nome, cognome, cf, telefono, email, partitaiva, username FROM gestoreagenziaimmobiliare WHERE username = ?";
        Map<String, Object> gestoreData = new HashMap<>();

        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, username);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                gestoreData.put("nome", rs.getString("nome"));
                gestoreData.put("cognome", rs.getString("cognome"));
                gestoreData.put("cf", rs.getString("cf"));
                gestoreData.put("telefono", rs.getString("telefono"));
                gestoreData.put("email", rs.getString("email"));
                gestoreData.put("partitaiva", rs.getString("partitaiva"));
                gestoreData.put("username", rs.getString("username"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
