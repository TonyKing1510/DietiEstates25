...
public class NotDaoImpl implements NotDao {
    ...
    public List<Notifica> getNot() {
        ...
        String query = "SELECT titolo,contenuto,id_notifica FROM notifica n join gestoreagenziaimmobiliare g on n.gestorenotificato=g.cf where g.partitaIva = ? and g.accettata = false";
    ...
    public NotificaDTO setNotificaRejected(int idNotifica) {
        String sql = "UPDATE notifica SET accettata = false WHERE id_notifica = ?";
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            preparedStatement.setInt(1, idNotifica);
            int nRows=preparedStatement.executeUpdate();
            if(nRows > 0){
