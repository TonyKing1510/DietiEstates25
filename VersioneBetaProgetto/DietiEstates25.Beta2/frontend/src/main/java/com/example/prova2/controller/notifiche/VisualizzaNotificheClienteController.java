...
public class VisualizzaNotificheClienteController extends VisualizzaNotificheAgenteController {
    ...
    private Pane creaNotificaPaneCliente(GetNotificheDTO notifica) {
        System.out.println(notifica.getIdNotifica());
        System.out.println(notifica.getCategoria());
        System.out.println(notifica.getNomeNotifica());
        notificationIndex++;
        Pane pane = new Pane();
        ...
        Label visualizzaDiPiu = FactoryNotificheCliente.createLabelVisualizzaDiPiu(this, notifica);
    ...
    private static void apriPaginaPrenotazione(Button selezionaUnAltroOrarioButton, Immobile immobile) {
        try {
            EffettuaPrenotazione.initPage((Stage) selezionaUnAltroOrarioButton.getScene().getWindow(),
                    clienteNotifiche.getAccountAgente().getEmail(), immobile);
        } catch (IOException e) {
            e.printStackTrace();
