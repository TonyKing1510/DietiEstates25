...
public class ModificaProfiloAgenteController implements ChangePasswordInterface {
    ...
    public void salvaDati() {
        if(checkCampi()){
            ...
            ModificaProfiloFacade.updateDatiAgente(dati);
    ...
    public boolean checkCampi() {
        ...
        flag&=checkDuplicatoEmail(textFieldEmail.getText().trim());
        return flag;
    }
    ...
    private void setFotoProfiloAsincrono() {
        if (DashboardAgenteController.getAgente().getFotoProfilo() != null) {
            setFotoProfilo(DashboardAgenteController.getAgente().getFotoProfilo().getPath());
        }else{
            setFotoProfilo("com/example/prova2/images/fotoOminoFB.jpg");
        }
    }
    ...
    private void aggiornaAgente(String url) {
        utente.setFotoProfilo(new Foto(url));
        setUtente(utente);
    }
